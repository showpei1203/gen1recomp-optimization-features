#!/usr/bin/env python3
"""Emit SoulGold G2 Cyndaquil Rich Ambient frame/action descriptors.

G2 emits one-shot ambient actions plus an explicit HOME action that reuses the
first Idle frame. The renderer remains the same two-slot rolling-cache model.
"""
...
def main() -> int:
    ...
    if missing:
        raise SystemExit(f"IR missing required G2 actions: {missing}")

    lines = [
        "/* Auto-generated SoulGold G2 Rich Ambient descriptors. */",
        "#include \"global.h\"",
    ...
    print(f"Wrote {args.output} (G2 Rich Ambient, variant={args.variant})")
    return 0
