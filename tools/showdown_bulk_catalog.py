...
This is the authority input for bulk migration. It does not patch the ROM.
It measures exact filename coverage, GIF frame counts and raw 64x64 4bpp
budget so full-Pokedex integration is based on evidence instead of optimism.
"""
...
def zip_lane_index(zf: zipfile.ZipFile) -> dict[str, dict[str, str]]:
    out = {lane: {} for lane in LANE_DIRS}
    ...
    for name in zf.namelist():
        ...
        # Exact final-directory indexing prevents ani/foo.gif from accidentally
        # resolving to gen5ani/foo.gif.
        if stem not in out[lane]:
            out[lane][stem] = name
    return out
...
def main() -> int:
    ...
    constants_path = soulgold / "include" / "constants" / "species.h"
    constants = parse_species_constants(constants_path)
    ...
    with zipfile.ZipFile(args.sprites_zip) as zf:
        ...
        for slug in host_species:
            ...
            row["runtime_exact_candidate"] = bool(row["normal_pair"] and row["species_constant_exists"])
            row["raw_4bpp_bytes_all_present_lanes"] = species_raw
            raw_total += species_raw
            rows.append(row)

    candidates = [r for r in rows if r["runtime_exact_candidate"]]
