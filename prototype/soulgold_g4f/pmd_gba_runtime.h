...
{
    const void *gfx;
    const struct PmdG4fPackedFrame *packed;
    u16 duration;
    s8 presentationX;
    s8 presentationY;
};
...
/* G4F: all frame decompression/reconstruction happens here. SoulGold calls this
 * before AnimateSprites(), outside the proven AnimateSprites -> PMD Tick ->
 * BuildOamBuffer presentation-critical window. */
