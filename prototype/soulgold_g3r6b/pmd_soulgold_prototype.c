...
#define PMD_G3R6B_AMBIENT_COUNT 4
...
struct PmdSpeciesProfile
{
    ...
    const struct PmdGbaAction *ambient[PMD_G3R6B_AMBIENT_COUNT];
    const struct PmdGbaAction *hurt;
    const struct PmdGbaAction *attack;
    const struct PmdSoulGoldShadowAction *shadowHome;
    const struct PmdSoulGoldShadowAction *shadowAmbient[PMD_G3R6B_AMBIENT_COUNT];
...
static void BindCurrentAmbient(u8 battler, const struct PmdSpeciesProfile *profile)
{
    ...
    action = profile->ambient[state->sequenceIndex % PMD_G3R6B_AMBIENT_COUNT];
...
static bool32 BindMoveAttack(u8 battler, const struct PmdSpeciesProfile *profile)
{
    struct PmdPresentationState *state = &sState[battler];

    if (profile == NULL || profile->attack == NULL)
        return FALSE;

    /* SoulGold's native move script remains spatial owner. PMD owns only the
     * body pixels and matching authored shadow timeline while DoMoveAnim runs. */
    PmdSoulGold_SetReactivePresentation(battler, TRUE);
    PmdSoulGold_SetNativeSpatialOwnership(battler, TRUE);
    if (!PmdGbaRuntime_Bind(battler, profile->attack))
    {
        PmdSoulGold_SetNativeSpatialOwnership(battler, FALSE);
        PmdSoulGold_SetReactivePresentation(battler, FALSE);
        return FALSE;
...
static const struct PmdSoulGoldShadowAction *GetCurrentShadowAction(const struct PmdPresentationState *state)
{
    ...
    if (state->phase == PMD_PHASE_HOME || state->phase == PMD_PHASE_HURT_RETURN || state->phase == PMD_PHASE_MOVE_RETURN)
        return state->profile->shadowHome;
    return state->profile->shadowAmbient[state->sequenceIndex % PMD_G3R6B_AMBIENT_COUNT];
}
...
void PmdSoulGoldPrototype_EndMoveAction(enum BattlerId battler)
{
    ...
    /* End of gAnimScriptActive is presentation ownership, not damage timing.
     * Return to HOME while retaining reactive/native-spatial gates until HOME
     * has actually replaced the final Attack pixels in OBJ VRAM. */
    BindHome(battler, profile, 28, TRUE);
    state->phase = PMD_PHASE_MOVE_RETURN;
    PmdSoulGold_SetReactivePresentation(battler, TRUE);
    PmdSoulGold_SetNativeSpatialOwnership(battler, TRUE);
}
...
bool32 PmdSoulGoldPrototype_IsMoveReturnReady(enum BattlerId battler)
{
    ...
    if (state->phase != PMD_PHASE_MOVE_RETURN)
...
void PmdSoulGoldPrototype_Tick(void)
{
    ...
    for (battler = 0; battler < PMD_GBA_MAX_BATTLERS; battler++)
    {
        ...
        if (PmdGbaRuntime_ConsumeInterrupted(battler))
        {
            if (state->phase == PMD_PHASE_MOVE_ATTACK)
            {
                /* Native move remains authoritative. Do not restart Attack in
                 * the middle of its FX if PMD presentation was temporarily
                 * unavailable. EndMoveAction will still force clean HOME. */
            ...
            if (state->phase == PMD_PHASE_MOVE_RETURN)
            {
                /* Re-arm HOME and wait for native special/substitute ownership
                 * to finish; the controller return gate remains closed. */
                if (PmdGbaRuntime_Bind(battler, profile->home))
                {
                    state->profile = profile;
                    state->initialized = TRUE;
        ...
        {
            u8 completedIndex = state->sequenceIndex % PMD_G3R6B_AMBIENT_COUNT;
            state->sequenceIndex = (state->sequenceIndex + 1) % PMD_G3R6B_AMBIENT_COUNT;
