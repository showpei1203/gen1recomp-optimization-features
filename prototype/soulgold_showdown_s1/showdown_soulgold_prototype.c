...
extern const struct ShowdownGbaAction gShowdownCyndaquilBackIdleAction;
extern const struct ShowdownGbaAction gShowdownCyndaquilFrontIdleAction;
    ...
    for (battler = 0; battler < SHOWDOWN_GBA_MAX_BATTLERS; battler++)
    {
        ...
        if (!sBound[battler])
        {
            action = GetBattlerSide(battler) == B_SIDE_PLAYER
                ? &gShowdownCyndaquilBackIdleAction
                : &gShowdownCyndaquilFrontIdleAction;
