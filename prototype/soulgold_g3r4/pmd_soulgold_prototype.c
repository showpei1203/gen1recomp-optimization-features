...
#define PMD_G3R4_ACTION_COUNT 4
...
struct PmdSpeciesProfile
{
    ...
    const struct PmdGbaAction *actions[PMD_G3R4_ACTION_COUNT];
    u16 homeHolds[PMD_G3R4_ACTION_COUNT];
};
...
static void BindCurrentAmbientAction(u8 battler, const struct PmdSpeciesProfile *profile)
{
    ...
    action = profile->actions[state->sequenceIndex % PMD_G3R4_ACTION_COUNT];
...
void PmdSoulGoldPrototype_Tick(void)
{
    ...
    for (battler = 0; battler < PMD_GBA_MAX_BATTLERS; battler++)
    {
        ...
        {
            u8 completedIndex = state->sequenceIndex % PMD_G3R4_ACTION_COUNT;
            state->sequenceIndex = (state->sequenceIndex + 1) % PMD_G3R4_ACTION_COUNT;
